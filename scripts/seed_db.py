    ...
    try:
        ...
        games1 = [
            Game(
                match_id=match1.id,
                game_type=GameType.NINE_BALL,
                winner_user_id=fast_eddie.id,
                loser_user_id=minnesota_fats.id,
                frame_number=1,
            ),
            Game(
                match_id=match1.id,
                game_type=GameType.NINE_BALL,
                winner_user_id=minnesota_fats.id,
                loser_user_id=fast_eddie.id,
                frame_number=2,
            ),
            Game(
                match_id=match1.id,
        ...
        db.commit()
        print(
            f"   ✓ Match 1: {fast_eddie.display_name} vs {minnesota_fats.display_name} (APPROVED, 9-ball race to 5)"
        )
        ...
        games2 = [
            Game(
                ...
                game_type=GameType.EIGHT_BALL,
                winner_user_id=fast_eddie.id,
                loser_user_id=vincent.id,
                frame_number=1,
            ),
            Game(
                match_id=match2.id,
                game_type=GameType.EIGHT_BALL,
                winner_user_id=vincent.id,
                loser_user_id=fast_eddie.id,
                frame_number=2,
            ),
            Game(
        ...
        db.commit()
        print(
            f"   ✓ Match 2: {fast_eddie.display_name} vs {vincent.display_name} (PENDING, 8-ball race to 7)"
        )
        ...
        db.commit()
        print(
            f"   ✓ Match 3: {vincent.display_name} vs {minnesota_fats.display_name} (APPROVED, 10-ball race to 3)"
        )
